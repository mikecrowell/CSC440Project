...
function CreateDialog(BodyElement, sender) {
    ...
    if (BodyElement != null) {
       ...
       // changed to actually display the download button
       // var downloadButton = document.createElement("IMG");
       // downloadButton.alt = "Download";
       // downloadButton.title = "Download";
       // downloadButton.style.cssText = "position:absolute;right:-15px;bottom:-25px;Width:75px;Height:25px;cursor:pointer";
       // downloadButton.onclick = function () {  }

       //  var downloadLink = document.createElement("IMG")
       // downloadLink.style.cssText = "position:absolute;right:-15px;bottom:-25px;cursor:pointer;Width:75px;Height:25px;";
       // downloadLink.innerHTML = downloadLink.title;
       // downloadLink.title = imgURL;
       // downloadLink.href = imgURL;
       // Brings up Dialog to download a file, but not the actual file
       // downloadLink.onclick = function () { window.location.href = "data:file/octet-stream," + imgURL; }
        
        var lblFileName = document.createElement("label");
        var fileName = imgURL.substring(imgURL.lastIndexOf("/") + 1, imgURL.length);
        lblFileName.innerHTML = fileName;
        lblFileName.style.cssText = ";color:blue;";
    

        dialog.appendChild(closeIcon);
        // dialog.appendChild(downloadButton);
        // dialog.appendChild(downloadLink);
        dialog.appendChild(lblFileName);
